...
Runtime shape checking via [beartype](https://github.com/beartype/beartype) is enabled by default in debug mode. Disable it for training runs with the `-O` flag:

```bash
uv run python -O grokking/cli.py
```
